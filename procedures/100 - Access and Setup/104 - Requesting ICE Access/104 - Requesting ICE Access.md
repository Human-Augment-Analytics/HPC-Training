...
## What you will need before you start the form
...
The form ends with a set of acknowledgments you check off before submitting. The ones worth knowing about in advance:

- ICE is for educational use only, and only students, instructors, TAs, and IT support staff may access the system.
- PACE does not support students directly. Students cannot open tickets with PACE; instructors, TAs, or local IT staff are expected to handle student-facing support.
- At least one instructor or TA must attend an ICE orientation session before access is granted. PACE coordinates scheduling with the course instructor.
- PACE cannot stop users from changing permissions on their own files, which can expose them to other users. PACE is not responsible for FERPA issues that result from that.
- PACE runs a 3-day maintenance window every 3 months. ICE, including file access, is unavailable during that window.
- Uploaded data is subject to the retention policy described in [KB0042094](https://gatech.service-now.com/home?id=kb_article_view&sysparm_article=KB0042094), and instructors are expected to pass that along to students and TAs.
